...
public class CalclulateElectricityBillServlet  extends HttpServlet{
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		Float rate = (Float) req.getAttribute("rate");
		int reading = currentReading - previousReading;
		
		Float amount = reading * rate;
		
		resp.setContentType("text/html");
		PrintWriter writer = resp.getWriter();
		
		writer.append("Previous reading is = ").append(previousReading.toString()).append("<br>")
		.append("Current reading is = ").append(currentReading.toString()).append("<br>").append("Rate for unit = " )
		.append(rate.toString()).append("<br>").append("Bill per this month is = ").append(amount.toString())
		.append("<br>");
		;
